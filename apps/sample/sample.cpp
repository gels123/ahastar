...
{
	char tempStr[255];
	sprintf(tempStr, "Simulation time elapsed: %1.2f, Display Time: %1.2f Experiment: %i",
					us->getSimulationTime(), us->getDisplayTime(), expnum);
...
void myDisplayHandler(unitSimulation *unitSim, tKeyboardModifier mod, char key)
{
	...
	{
		case '\t': 
			if(dynamic_cast<AnnotatedClusterAbstraction*>(unitSim->getMapAbstraction()) != 0)
			{
				AnnotatedClusterAbstraction* aca = dynamic_cast<AnnotatedClusterAbstraction*>(unitSim->getMapAbstraction());
				if(aca->getDrawClusters()  == false)
				{
					if(aca->getDrawAbstractGraph() == false)
						aca->setDrawClusters(true);
					else
						aca->setDrawAbstractGraph(false);
				}
				else
				{
					if(aca->getDrawClusters() == true && aca->getDrawAbstractGraph() == true)
					{
						aca->setDrawClusters(false);
						aca->setDrawAbstractGraph(false);
					}
					else
					{
						aca->setDrawAbstractGraph(true);
					}
				}
